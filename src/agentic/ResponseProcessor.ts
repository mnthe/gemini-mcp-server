...
export class ResponseProcessor {
   ...
   */
  private extractReasoning(response: string): ReasoningItem[] {
  ...
  validate(response: string): void {
    ...
    if (hasToolCall && !hasArguments) {
      throw new ModelBehaviorError(
        response,
        'Malformed tool call: TOOL_CALL found without ARGUMENTS'
      );
    }
    ...
    if (!hasToolCall && hasArguments) {
      throw new ModelBehaviorError(
        response,
        'Malformed tool call: ARGUMENTS found without TOOL_CALL'
      );
    }
  }
