  ...
  async run(
    ...
    while (state.canContinue()) {
      ...
      try {
        ...
        state.logger.error('Invalid response from model', error as Error);
...
2. Your final answer if you have enough information`;
   ...
   */
  private shouldUseThinking(state: RunState): boolean {
   ...
   */
  private buildFallbackPrompt(state: RunState, toolCalls: any[], results: any[]): string {
...
[Note: This response may be incomplete due to execution limits. Please try rephrasing your question or breaking it into smaller parts.]`;
   ...
   * Build result object
   */
  private buildResult(state: RunState, finalOutput: string): RunResult {
   ...
   */
  private buildBestEffortResult(state: RunState, invalidResponse: string): RunResult {
