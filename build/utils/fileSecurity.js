...
import { SUPPORTED_IMAGE_TYPES, SUPPORTED_VIDEO_TYPES, SUPPORTED_AUDIO_TYPES, SUPPORTED_DOCUMENT_TYPES } from '../types/multimodal.js';
/**
 ...
 */
function getDefaultSafeDirectories() {
    const dirs = [
    ...
    if (homeDir) {
        ...
        dirs.push(path.join(homeDir, 'Documents'), path.join(homeDir, 'Downloads'), path.join(homeDir, 'Desktop'));
    }
    return dirs;
}
/**
...
export function validateMimeType(mimeType) {
    if (!SAFE_MIME_TYPES.has(mimeType)) {
        throw new SecurityError(`Unsupported or unsafe MIME type: ${mimeType}. Only images, videos, audio, and documents are allowed.`);
    }
}
/**
...
export function validateFileExtension(filePath) {
    ...
    if (EXECUTABLE_EXTENSIONS.has(ext)) {
        throw new SecurityError(`Executable file type not allowed: ${ext}. Only media and document files are permitted.`);
    }
}
/**
...
export function validateFilePath(filePath, config = {}) {
    ...
    if (!isInSafeDirectory) {
        throw new SecurityError(`File path is outside allowed directories. File: ${absolutePath}. ` +
 ...
 */
export function validateFileUri(fileUri, config = {}) {
    ...
    if (fileUri.startsWith('http://')) {
        throw new SecurityError('HTTP URIs are not allowed. Use HTTPS for web resources or gs:// for Cloud Storage.');
    }
    ...
    throw new SecurityError(`Unsupported URI scheme in: ${fileUri}. Only gs://, https://, and file:// are allowed.`);
}
/**
 ...
 */
export function validateMultimodalFile(mimeType, fileUri, config = {}) {
    // Validate MIME type
...
//# sourceMappingURL=fileSecurity.js.map